...
{
    public static List<TowerModel> TowerBuffed = new();
    ...
    public static void GiftOfGivingHandler(bool makeActive)
    {
        ...
        var rateAdd = makeActive ? -0.15f : 0.15f;
        var rangeAdd = makeActive ? 15f : -15f;

        for (var i = XmasMod2025.GiftOfGivingTowersIds.Count - 1; i >= 0; i--)
        {
    ...
    public static void FestiveSpiritHandler()
    {
        ...
        if (XmasMod2025.FestiveSpiritTower == null)
            InGame.instance.bridge.CreateTowerAt(new Vector2(0, 0), ModContent.GetTowerModel<FestiveSpiritTower>(),
                ObjectId.Create(0), false, callback, true, true, false, 0, false);
    }

    public static void SantaHelperHandler()
    {
        Action<bool> callback = _ => { };
        InGame.instance.bridge.CreateTowerAt(new Vector2(0, 0), ModContent.GetTowerModel<ElfSpawner>(),
            ObjectId.Create(0), false, callback, true, true, false, 0, false);
    }
}